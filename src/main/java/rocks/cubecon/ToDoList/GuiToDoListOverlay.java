...
public class GuiToDoListOverlay extends Gui
{
	...
	public void renderOverlay(RenderGameOverlayEvent event)
	{
		if(showGui && event.getType() == RenderGameOverlayEvent.ElementType.TEXT)
		{
            ...
            ToDoList.customFontRenderer.drawString(TextFormatting.ITALIC + GuiConfig.headingText, currentImageWidth, (sr.getScaledHeight() - 60) / 4, 3);
			GlStateManager.popMatrix();
			GlStateManager.pushMatrix();
			GlStateManager.scale(1, 1, 1);
            ToDoList.customFontRenderer.drawSplitString(GuiConfig.task1Text, currentImageWidth + 5, (sr.getScaledHeight() / 2) - 8,120, 0);
            ToDoList.customFontRenderer.drawSplitString(GuiConfig.task2Text, currentImageWidth + 5, (sr.getScaledHeight() / 2) + 18,120, 0);
            ToDoList.customFontRenderer.drawSplitString(GuiConfig.task3Text, currentImageWidth + 5, (sr.getScaledHeight() / 2) + 44, 120, 0);
            ToDoList.customFontRenderer.drawSplitString(GuiConfig.task4Text, currentImageWidth + 5, (sr.getScaledHeight() / 2) + 70, 120, 0);
			this.zLevel = 300.0f;
			mc.getTextureManager().bindTexture(backgroundLocation);
	...
	private void handleStrokeLine(StrokeLine strokeLine, int posY, float partialTicks)
    {
		...
		if (strokeLine.isShouldDraw() && strokeLine.getCurrentLength() <= strokeLine.getMaxLength())
		{
			...
			if(strokeLine.getMaxLength() > 120 && strokeLine.getCurrentLength() >= 120)
			{
				...
				drawTexturedModalRect(currentImageWidth + 4, posY + ToDoList.customFontRenderer.FONT_HEIGHT, 0, 254, strokeLine.getCurrentLength()-120, 2);
			}
			else
			{
    ...
    {
        strokeLineTask1.setMaxLength(ToDoList.customFontRenderer.getStringWidth(GuiConfig.task1Text) + 1);
        strokeLineTask2.setMaxLength(ToDoList.customFontRenderer.getStringWidth(GuiConfig.task2Text) + 1);
        strokeLineTask3.setMaxLength(ToDoList.customFontRenderer.getStringWidth(GuiConfig.task3Text) + 1);
        strokeLineTask4.setMaxLength(ToDoList.customFontRenderer.getStringWidth(GuiConfig.task4Text) + 1);
    }
}
